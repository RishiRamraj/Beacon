...
COURTYARD = {
  ...
  { tx = 159, ty = 475, room = 0x72, level = 0, kind = "gate", gate = {"any", {"tile_outside", 0xF0, 0xFF}, {"keys"}},
    note = "The locked south door out of 0x72, upper floor: a 2x2 at 159-160,475-476 reading 0xF8 while shut. The room guard drops its key, which is why the clear step above comes first. gate: an OPEN door is always a target so the guide leads through it; a shut one only once Link holds a key, otherwise the guide stays on the fight and the chest that holds it." },
  ...
  { tx = 104, ty = 495, room = 0x71, level = 1,
    note = "Lower-floor anchor in room 0x71 east pit, where the key-carrying soldier stands. The pits are walled off from each other and the reachability fill bounds the enemy tally to the one Link is in, so standing here is what makes that soldier count and lets the guide lead to it - from the west pit it was unreachable and nothing was offered. Moved here from 88,495 in the west pit, which is why an older note said \"by the chest\": the chest is in the west pit, not this one." },
  { tx = 79, ty = 486, room = 0x71, level = 0, kind = "gate", gate = {"keys"},
    note = "The locked door itself, up on the UPPER floor (a 2x2 at 79-80,485-486). Room 0x71 is open enough on the lower floor that the pathfinder can reach the far waypoint without ever crossing this door, so a pure collision block is not enough and the guide has to be told the dependency. gate: only a target once Link holds a key to open it, and kind gate retires it once the door tile stops reading as locked. NOTE: the approach described here previously - a clean straight climb up the swap stair and north with no floor-flip back to L1 - was from the WEST anchor at 88,495. Waypoint 13 now sits in the east pit on the lower floor, so the chain crosses east to west and changes floor between them; that route has not been walked since the move. If the guide behaves oddly between 13 and 14, start here." },
  ...
  { room = 0x80, kind = "enemy", via = true, carries = "key",
    note = "The Ball-and-Chain Trooper here carries the big key, so it is the fight that matters rather than the room at large: kind enemy with carries key targets the one that still drops a key on death (die_action non-zero), so a respawned guard in an already-looted room does not re-arm it. Zelda is in this room too and reads as a sprite, which is why the step names an enemy rather than counting bodies." },
...
SANCTUARY = {
  ...
  { tx = 156, ty = 74, room = 0x12, level = 0, kind = "chest", arrival = "Well excuuuuuse me, princess.", via = true, quiet = true,
    note = "The Sanctuary chest, a 2x2 at 156-157,74-75, named by its top-left tile as the other chest steps are. kind chest retires it once the tile stops reading as a chest, and having a done is what puts it in the errand index, so it stays reachable after the escort chain retires at progress 2 - which is why the arrival line lives here rather than on the door below. via, because the door sits later in the chain and the driver picks the furthest reachable step in a room." },
  { tx = 159, ty = 120, room = 0x12, level = 0,
    note = "The Sanctuary door, attr 0x8E, TileBehavior_Entrance: the doorway Link brings Zelda through, and the chain last step. A plain position with no done, so it is not errand-indexed and is only reachable while the escort chain is armed." },
},
